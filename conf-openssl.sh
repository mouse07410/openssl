...
make update && make depend && make clean && make -j 2 all 2>&1 | tee make-out.txt && OPENSSL_GOST_ENGINE_SO="${OPENSSL_GOST_ENGINE_SO}" make test 2>&1 | tee test-out.txt  && make install
